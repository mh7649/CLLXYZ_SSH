...
function showEditWin() {
    var jsonStr = '{"message.id":"id_value","message.content":"content_value"}';
...
function updateMessage() {
    ...
    if (row) {
        $.post(contextPath + "/message/updateMessage",
            $("#editForm").serialize(),
            function(data) {
...
function valid(status) {
    ...
    if (row) {
        if (row.status == status) {
            showAlert("无需" + (status == 1 ? "激活" : "冻结") + "该留言！");
            return;
        }
        var json;
        if (status == 1) {
        ...
        $.post(contextPath + "/message/statusMessage ",
            json,
            function (data) {
                if (data.controllerResult.result == "success") {
                    showAlert(data.controllerResult.message);
        ...
        showAlert("请选择需要" + (status == 1 ? "激活" : "冻结") + "的留言！");
    }
}
